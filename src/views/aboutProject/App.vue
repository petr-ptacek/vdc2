<script setup lang="ts">
import { WAppHeader } from "@/components";
</script>

<template>
	<div class="w-full h-full flex flex-col">
		<WAppHeader class="relative">
			<div class="text-[3rem] font-semibold absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
				O Projektu
			</div>
		</WAppHeader>

		<main class="flex-1 bg-brown-semi p-4">
			<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Accusantium amet asperiores at atque, blanditiis
				 consectetur culpa doloribus dolorum earum est facere facilis fugiat, fugit harum id illum inventore iste magnam
				 magni maxime molestiae nam odio optio perferendis possimus quae qui repellat reprehenderit sed soluta sunt
				 totam vel velit! Iste, rem!</p>
		</main>
	</div>
</template>
